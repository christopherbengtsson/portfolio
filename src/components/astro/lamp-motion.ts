...
export function initLampMotion(
  root: HTMLButtonElement,
  getTheme: () => Theme,
  setTheme: (theme: Theme) => void,
): { sync(): void; cleanup(): void } {
  const scenes = [...root.querySelectorAll<SVGSVGElement>('[data-lamp-scene]')].map((svg) => ({
  ...
  function visible() {
    ...
    return !root.hidden && bounds.width > 0 && bounds.height > 0
  ...
  function draw() {
    ...
    for (const { cordX, cordY, string, knob } of scenes) {
      string.setAttribute('d', `M ${cordX} 29 C ${cordX - .5} 36 ${cordX + .4} ${cordY - 5 + pull} ${cordX} ${cordY + pull}`);
    ...
    arm.setAttribute('d', `M 49 48 C 60 ${52 - 5 * reach} ${handX - 12} ${handY + 3 * reach} ${handX} ${handY}`);
  ...
  function sync() {
    ...
    pointerReach = 0;
    pointerDirty = false;
    reach = !reduced.matches && desktop.matches && keyboardReady && visible() ? 1 : 0;
    press = 0;
  ...
  function commit(current: Activation, now: number) {
    ...
    if (activation === current) {
      root.dataset.lampOn = String(getTheme() === 'light');
      illumination = { startedAt: now, from: light, to: current.target === 'light' ? 1 : 0 };
      requestFrame();
    }
  }
  ...
  function updatePointer() {
    ...
    const distance = Math.hypot(
      pointer.x - (bounds.left + bounds.width * .65),
      pointer.y - (bounds.top + bounds.height * .65),
    );
  ...
  function tick(now: number) {
    ...
    if (Math.abs(target - reach) < .002) reach = target;
    press = 0;
    draw();
  ...
  root.addEventListener('pointerdown', () => { keyboardReady = false; }, { signal });
  root.addEventListener('keydown', () => {
    keyboardReady = root.matches(':focus-visible');
    if (keyboardReady && !reduced.matches) requestFrame();
  }, { signal });
  document.addEventListener('pointermove', (event) => {
    if (event.pointerType === 'touch' || reduced.matches || !desktop.matches || !finePointer.matches || !inView) return;
