...
export function initSiteInteractions() {
  ...
  if (!reduced.matches && 'IntersectionObserver' in window) {
    ...
    for (const element of targets) {
      if (element.getBoundingClientRect().top >= innerHeight && !destination?.contains(element)) {
  ...
  const language = document.querySelector<HTMLDetailsElement>('.landing-language details');
  if (!language) return;
  ...
  document.addEventListener('click', (event) => {
    if (event.target instanceof Node && !language.contains(event.target)) language.open = false;
  });
  document.addEventListener('focusin', (event) => {
    if (event.target instanceof Node && !language.contains(event.target)) language.open = false;
  });
